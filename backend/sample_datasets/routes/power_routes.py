...
async def visualize(req: SessionRequest):
...
async def detect_pii(req: SessionRequest):
...
async def preview_formula(req: FormulaPreviewRequest):
    """
    ...
    try:
        ...
        if hasattr(result, "tolist"):
            preview_vals = [str(v) if not pd.isna(v) else "NaN" for v in result.tolist()]
...
@router.post("/formula")
async def add_formula_column(req: FormulaRequest):
    """
    ...
    try:
        ...
        session.push_history(df_before, "formula_column",
                             {"new_column": new_col, "expression": expr})
        session.df_current = df
        persist_dataset(req.session_id, df, session.filename)
