...
from utils.billing import (
    get_tier, set_tier, get_usage_today, get_dataset_count, TIERS
)
...
def my_billing(user: AuthUser = Depends(get_current_user)):
    ...
    row       = db.fetchone("SELECT tier, stripe_sub_id FROM user_tiers WHERE user_id = ?",
                            (user.user_id,))
...
def create_checkout(req: UpgradeRequest, user: AuthUser = Depends(get_current_user)):
    ...
    try:
        ...
        row = db.fetchone("SELECT stripe_customer_id FROM user_tiers WHERE user_id = ?",
                          (user.user_id,))
...
async def stripe_webhook(request: Request):
    ...
    if event_type == "checkout.session.completed":
        ...
        if row:
            ...
            logger.info(f"Billing: downgraded user={row['user_id']} → free (sub cancelled)")
...
def cancel_subscription(user: AuthUser = Depends(get_current_user)):
    ...
    row = db.fetchone("SELECT stripe_sub_id FROM user_tiers WHERE user_id = ?", (user.user_id,))
    ...
    try:
        ...
        return JSONResponse({"cancelled": True, "note": "Access continues until end of billing period."})
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
