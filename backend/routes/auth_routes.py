...
def _enforce_auth_rate_limit(ip: str) -> None:
    """Skip in-process auth throttling for FastAPI's in-memory test client."""
    if ip == "testclient":
        return
...
class AuthRequest(BaseModel):
    ...
    # Registration-only fields (ignored on login)
...
def _register(req: AuthRequest):
    ...
    if req.email:
        existing_email = db.fetchone("SELECT id FROM users WHERE email = ?", (req.email,))
        if existing_email:
            raise AuthConflictError("Email address already in use.")
    user_id  = str(uuid.uuid4())
    pw_hash  = _hash_password(req.password)
    db.execute(
        "INSERT INTO users (id, username, password_hash, role, full_name, email) VALUES (?, ?, ?, ?, ?, ?)",
        (user_id, req.username, pw_hash, "user", req.full_name or None, req.email or None),
    )
    logger.info(f"Auth: registered user '{req.username}' id={user_id} email={req.email}")
...
def me(user: AuthUser = Depends(get_current_user)):
    ...
    row = _db.fetchone(
        "SELECT full_name, email FROM users WHERE id = ?", (user.user_id,)
    )
    return JSONResponse({
