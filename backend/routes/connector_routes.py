...
def _cap_query(query: str, limit: int) -> str:
    ...
    q = re.sub(
        r"\bFETCH\s+(FIRST|NEXT)\s+\d+\s+ROWS?\s+ONLY\s*$", "", q, flags=re.IGNORECASE
    ).rstrip()
...
async def connect_url(
    req: URLConnectorRequest, user: AuthUser = Depends(get_current_user)
):
    ...
    try:
        ...
        filename = (
            req.filename or req.url.split("/")[-1].split("?")[0] or "imported.csv"
        )
...
async def connect_gsheets(
    req: GSheetsRequest, user: AuthUser = Depends(get_current_user)
):
    ...
    try:
        ...
        save_session(session_id, session)
        logger.info(
            f"GSheets connector: {req.spreadsheet_id}/{ws.title} → session={session_id}"
        )
...
async def connect_s3(
    req: S3ConnectorRequest, user: AuthUser = Depends(get_current_user)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=501, detail="S3 connector requires: pip install boto3"
        )
    except Exception as e:
...
async def connect_database(
    req: DatabaseConnectorRequest, user: AuthUser = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        session = DatasetSession(
            df=df, filename=req.filename or "db_query.csv", owner_id=user.user_id
        )
        save_session(session_id, session)
