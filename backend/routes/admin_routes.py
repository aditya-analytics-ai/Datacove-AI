...
def deactivate_user(user_id: str, admin: AuthUser = Depends(require_admin)):
    ...
    user = db.fetchone("SELECT id, is_active FROM users WHERE id = ?", (user_id,))
    if not user:
        raise HTTPException(status_code=404, detail="User not found.")
...
def delete_user(user_id: str, admin: AuthUser = Depends(require_admin)):
    if user_id == admin.user_id:
    ...
    return JSONResponse({"success": True, "deleted": user_id})


class BulkDeleteByPattern(BaseModel):
    pattern: str  # SQL LIKE pattern, e.g. "testuser_%"


@router.post("/admin/users/bulk-delete-by-pattern")
def bulk_delete_by_pattern(
    body: BulkDeleteByPattern,
    admin: AuthUser = Depends(require_admin),
):
    """Delete all users whose username matches a LIKE pattern, excluding the caller."""
    rows = db.fetchall(
        "SELECT id, username FROM users WHERE username LIKE ? AND id != ?",
        (body.pattern, admin.user_id),
    )
    if not rows:
        return JSONResponse({"success": True, "deleted": 0, "users": []})
...
def audit_log(
    ...
    total = db.fetchone(f"SELECT COUNT(*) as n FROM audit_log {where}", tuple(params))[
        "n"
    ]
    return JSONResponse(
        {
...
def platform_stats(admin: AuthUser = Depends(require_admin)):
    ...
    active_users = db.fetchone("SELECT COUNT(*) as n FROM users WHERE is_active = 1")[
        "n"
    ]
    admin_count = db.fetchone("SELECT COUNT(*) as n FROM users WHERE role = 'admin'")[
        "n"
    ]
