...
async def run_agent(req: AgentRequest, user: AuthUser = Depends(get_current_user)):
    """
...
async def nl_clean(req: NLCommandRequest, user: AuthUser = Depends(get_current_user)):
    """
...
def list_versions(session_id: str, user: AuthUser = Depends(get_current_user)):
