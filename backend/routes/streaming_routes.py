...
async def stream_clean(
    req: StreamCleanRequest, user: AuthUser = Depends(get_current_user)
):
    ...
    if len(session.df_current) < settings.STREAM_THRESHOLD or not is_streamable(
        req.action
    ):
    ...
    async def _stream() -> AsyncGenerator[str, None]:
        def _run():
            return list(
                stream_transform(
                    req.session_id, source_path, req.action, req.params, req.chunk_size
                )
            )
        ...
        if out_holder:
            try:
                ...
                # Save version snapshot only after successful transform
                version = len(session.versions) + 1
...
async def stream_auto_clean_endpoint(
    req: StreamAutoCleanRequest, user: AuthUser = Depends(get_current_user)
):
