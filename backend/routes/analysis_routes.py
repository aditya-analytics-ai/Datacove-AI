...
@asynccontextmanager
async def _analysis_boundary(operation: str, session_id: str | None = None):
    """Translate unexpected analysis failures into consistent HTTP 500s."""
    try:
        yield
...
@router.post("/profile")
async def profile(req: SessionRequest, user: AuthUser = Depends(get_current_user)):
    """Return full dataset profile (with smart sampling on large datasets)."""
    async with _analysis_boundary("Profile", req.session_id):
...
@router.post("/analyze")
async def analyze(req: SessionRequest, user: AuthUser = Depends(get_current_user)):
    """Return issues + health score + anomalies + AI suggestions."""
    async with _analysis_boundary("Analyze", req.session_id):
        session = require_session(req.session_id, owner_id=user.user_id)
        df = session.df_current
...
@router.get("/summary")
async def summary(session_id: str, user: AuthUser = Depends(get_current_user)):
    """
    ...
    """
    async with _analysis_boundary("Summary", session_id):
        ...
        if session is None:
            ...
            return JSONResponse(
                {"session_id": session_id, "ready": False}, status_code=202
            )
        df = session.df_current
...
@router.post("/summary")
async def summary_post(req: SessionRequest, user: AuthUser = Depends(get_current_user)):
    """Backward-compatible POST wrapper for summary requests."""
    return await summary(req.session_id, user)


@router.post("/compare")
async def compare(req: CompareRequest, user: AuthUser = Depends(get_current_user)):
    """Compare two dataset sessions - column/row/cell-level diffs."""
    async with _analysis_boundary("Compare", f"{req.session_id_a},{req.session_id_b}"):
        session_a = require_session(req.session_id_a, owner_id=user.user_id)
        session_b = require_session(req.session_id_b, owner_id=user.user_id)
...
async def nl_command(req: NLCommandRequest, user: AuthUser = Depends(get_current_user)):
    ...
    try:
        ...
        result = await run_in_threadpool(
            parse_command, req.command, list(session.df_current.columns)
        )
        return JSONResponse(result)
    except HTTPException:
        raise
...
async def correlations(
    req: CorrelationRequest, user: AuthUser = Depends(get_current_user)
):
    """Pearson / Spearman / Cramér's V correlation heatmap."""
    try:
        session = require_session(req.session_id, owner_id=user.user_id)
        result = await run_in_threadpool(
            detect_correlations, session.df_current, req.method, req.threshold
        )
        return JSONResponse(result)
    except HTTPException:
        raise
...
async def referential_integrity(
    req: SessionRequest, user: AuthUser = Depends(get_current_user)
):
    """Auto-detect PK/FK columns; flag orphaned values, duplicate PKs, null FKs."""
    try:
        session = require_session(req.session_id, owner_id=user.user_id)
        result = await run_in_threadpool(
            check_referential_integrity, session.df_current
        )
        return JSONResponse(result)
    except HTTPException:
        raise
...
async def timeseries_anomalies(
    req: TimeseriesRequest, user: AuthUser = Depends(get_current_user)
):
...
from services.pattern_library import (
    list_patterns,
    validate_column as _validate_col,
    test_value,
)  # noqa
...
async def pattern_validate(
    req: PatternValidateRequest, user: AuthUser = Depends(get_current_user)
):
    """Validate a column against a named pattern."""
    try:
        session = require_session(req.session_id, owner_id=user.user_id)
        result = await run_in_threadpool(
            _validate_col, session.df_current, req.column, req.pattern_name
        )
        return JSONResponse(result)
    except HTTPException:
        raise
