...
from utils.errors import PipelineValidationError, PipelineNotFoundError
...
def create(req: CreatePipelineRequest, user: AuthUser = Depends(get_current_user)):
    try:
        pipeline = create_pipeline(req.name, req.steps, user.user_id)
        ...
        return JSONResponse(
            {
                ...
                "name": pipeline.name,
                "steps": [
                    {"action": s.action, "params": s.params} for s in pipeline.steps
                ],
            }
        )
...
def list_pipelines(user: AuthUser = Depends(get_current_user)):
    try:
        return JSONResponse(list_all_pipelines(user.user_id))
    except Exception as e:
...
async def run(req: RunPipelineRequest, user: AuthUser = Depends(get_current_user)):
    try:
        ...
        session.push_history(
            df_before, "run_pipeline", {"pipeline_id": req.pipeline_id}
        )
